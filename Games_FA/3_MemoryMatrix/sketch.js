...
const STRINGS = {
  ...
  instructionsCommon:
    ...
    'برای لغو انتخاب یک خانه، دوباره روی آن کلیک کنید.',
  moreCellsNote: 'در این مرحله باید تعداد خانه‌های بیشتری به خاطر سپرده شود.',
  ...
  famAnswerHint: 'خانه‌های درست با کادر مشخص شده‌اند؛ خانه‌های قرمز اشتباه کلیک شده‌اند.',
  famDemos: [
    'گام ۱ — به‌خاطرسپاری: چند خانه برای لحظه‌ای روشن می‌شود.\nدقیقاً به خاطر بسپارید کدام‌ها.',
    'گام ۲ — شبکه برای لحظه‌ای ناپدید می‌شود.\nالگو را در ذهن نگه دارید.',
    'گام ۳ — یادآوری: روی همهٔ خانه‌هایی که روشن بودند کلیک کنید\n(اینجا پاسخ با کادر مشخص شده)، سپس «پایان» را بزنید.\nبرای لغو انتخاب یک خانه، دوباره روی آن کلیک کنید.'
  ],
};
...
function drawRecallScreen() {
  ...
  if (nowMs() - recallOnsetMs >= effectiveWindowMs()) {
    finishTrial('timeout');
  }
}
...
function drawHUD(showCountdown) {
  ...
  if (showCountdown) remainingMs = max(0, effectiveWindowMs() - (nowMs() - recallOnsetMs));
...
function buildFamPlan(withDemos) {
  const F = CONFIG.FAMILIARIZATION;
  const plan = [];
  if (withDemos && F.DEMOS[0] > 0) {
    plan.push({ type: 'demo', level: 0, items: [
      { frame: 'stimulus',  caption: STRINGS.famDemos[0] },
      { frame: 'retention', caption: STRINGS.famDemos[1] },
      { frame: 'recall',    caption: STRINGS.famDemos[2] }
    ].slice(0, F.DEMOS[0]) });
  }
...
/* Correct-response hint: the participant's picks with the answer outlined. */
function drawFamAnswerHint() {
  const trial = trialPool[trialIdxLevel];
  // Cells clicked by mistake (selected but not targets) are over-painted red
  stroke(CONFIG.GRID.LINE);
  strokeWeight(2);
  fill(CONFIG.COLORS.INCORRECT);
  for (const key of selectedCells) {
    if (trial.targetSet.has(key)) continue;
    const parts = key.split(',');
    const row = Number(parts[0]), col = Number(parts[1]);
    rect(gridOriginX + col * cellSizePx + cellSizePx / 2,
         gridOriginY + row * cellSizePx + cellSizePx / 2,
         cellSizePx - 4, cellSizePx - 4, CONFIG.CELL_CORNER_RADIUS);
  }
  noFill();
  stroke(CONFIG.GRID.TARGET);
  strokeWeight(4);
  for (const key of trial.targetSet) {
    const parts = key.split(',');
    const row = Number(parts[0]), col = Number(parts[1]);
    rect(gridOriginX + col * cellSizePx + cellSizePx / 2,
         gridOriginY + row * cellSizePx + cellSizePx / 2,
         cellSizePx - 10, cellSizePx - 10, CONFIG.CELL_CORNER_RADIUS);
  }
  noStroke();
  fill(CONFIG.COLORS.HUD);
  textSize(18);
...
function drawFamTrialUnderlay() {
  drawGrid(trialPool[trialIdxLevel], false, true);   // grid + participant's selections
}
...
function drawFamDemoStimulus(item) {
  ...
  for (let row = 0; row < n; row++) {
    for (let col = 0; col < n; col++) {
      ...
      stroke(CONFIG.GRID.LINE);
      strokeWeight(2);
      fill(item.frame === 'stimulus' && targets.has(key) ? CONFIG.GRID.TARGET : CONFIG.GRID.EMPTY);
      rect(cx, cy, cellSizePx - 4, cellSizePx - 4, CONFIG.CELL_CORNER_RADIUS);
      if (item.frame === 'recall' && targets.has(key)) {   // outline the answer
        noFill();
        stroke(CONFIG.GRID.TARGET);
        strokeWeight(4);
        rect(cx, cy, cellSizePx - 10, cellSizePx - 10, CONFIG.CELL_CORNER_RADIUS);
      }
    }
  }
  if (item.frame === 'recall') {
    ...
    textSize(20);
    textStyle(BOLD);
    text(STRINGS.btnDone, width / 2, by - 1);
    textStyle(NORMAL);
  }
}
