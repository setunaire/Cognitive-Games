/* ============================================================================
   CHALKBOARD CHALLENGE — Assessment Phase (English version)
...
const CONFIG = {
  ...
  FAMILIARIZATION: {
    ...
    PRACTICE_WINDOW_MS: 10000,       // flat practice window, all levels — L3 is practiced (6000 ms floor) — thinnest margin in the battery
...
/* ============================================================================
   ...
   ========================================================================== */
const STRINGS = {
  gameTitle: 'Chalkboard Challenge',
  gameSubtitle: 'Which side has the larger value?',
  ...
  ofLabel: 'of',
  instructionsCommon:
    'Two calculations appear side by side.\n' +
    'Decide which one gives the LARGER result.\n' +
    '← = left is larger        → = right is larger',
  // Precedence reminders — shown only at levels whose operators need them
  precedenceNoteMult: 'Remember: × is computed before + and −.',
  precedenceNoteMultDiv: 'Remember: × and ÷ are computed before + and −.',
  ...
  famAnswerValues: 'Left = {l}        Right = {r}',
  famDemos: [
    'Two expressions appear — decide which side is LARGER.\nLeft: 47 + 5 = 52.  Right: 35 + 45 = 80 — press →.',
    '× and ÷ are computed BEFORE + and −.\n12 + 7 × 3  →  7 × 3 = 21  →  12 + 21 = 33.  Right: 42 — press →.'
  ],
...
/* ---------- Chalkboard: fixed expression demos (worked precedence at L2/L3) ---------- */
function famExpr(terms, ops) { return { terms: terms, ops: ops }; }

function buildFamPlan(withDemos) {
  const F = CONFIG.FAMILIARIZATION;
  const demoDefs = [
    [ { left: famExpr([47, 5], ['+']),        right: famExpr([35, 45], ['+']), caption: STRINGS.famDemos[0] } ],
    [ { left: famExpr([12, 7, 3], ['+','*']), right: famExpr([50, 8], ['-']),  caption: STRINGS.famDemos[1] } ],
    []
  ];
  const plan = [];
  if (withDemos) {
...
function famT(lTerms, lOps, lVal, rTerms, rOps, rVal) {
  return { leftExpr: { terms: lTerms, ops: lOps }, rightExpr: { terms: rTerms, ops: rOps },
           leftValue: lVal, rightValue: rVal,
           diffRatio: +((Math.abs(lVal - rVal) / Math.max(lVal, rVal)).toFixed(4)),
           correctResponse: lVal > rVal ? 'left' : 'right' };
}

const FAM_PRACTICE_SETS = [
  [ // set A — every trial inside its level's diff-ratio band; L2/L3 always
    //          carry the level's required operator on one side
    [ famT([41, 7], ['+'], 48,          [22, 3], ['+'], 25) ],
    [ famT([31, 5], ['+'], 36,          [6, 9], ['*'], 54),
      famT([20, 3, 4], ['+', '*'], 32,  [45, 3], ['+'], 48) ],
    [ famT([36, 4, 22], ['/', '+'], 31, [6, 3, 4], ['*', '+'], 22) ]
  ],
  [ // set B
    [ famT([12, 6], ['+'], 18,          [38, 9], ['+'], 47) ],
    [ famT([8, 7], ['*'], 56,           [34, 5], ['+'], 39),
      famT([19, 25], ['+'], 44,         [40, 2, 6], ['-', '*'], 28) ],
    [ famT([45, 5, 18], ['/', '+'], 27, [7, 4, 9], ['*', '+'], 37) ]
  ]
];
...
function drawFamDemoStimulus(item) {
  drawExpressionCards({ leftExpr: item.left, rightExpr: item.right });
}

/* Correct-response hint: both computed values plus the correct key. */
function drawFamAnswerHint() {
  ...
  textSize(24);
  textStyle(BOLD);
  text(fmtTemplate(STRINGS.famAnswerValues, { l: fmtNum(trial.leftValue), r: fmtNum(trial.rightValue) }), width / 2, 92);
  text(fmtTemplate(STRINGS.famAnswerWas, { a: trial.correctResponse === 'left' ? '←' : '→' }), width / 2, 128);
  textStyle(NORMAL);
}

/* The trial display shown underneath familiarization feedback. */
function drawFamTrialUnderlay() {
  drawExpressionCards(trialPool[trialIdxLevel]);
}
